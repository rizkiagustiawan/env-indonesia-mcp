...
pub fn calculate(scope1_json: &str, scope2_json: &str, scope3_json: &str) -> String {
    ...
    fn get_ef(source: &str, unit: &str) -> Option<(f64, &'static str)> {
        ...
        match key.as_str() {
            // Scope 1 - combustion
            "boiler_diesel_liter" | "diesel_liter" | "diesel_generator_liter" | "solar_liter" => {
                Some((2.68, "IPCC 2006: 2.68 kgCO2/L"))
            }
            "boiler_gas_m3" | "natural_gas_m3" | "gas_m3" => {
                Some((2.02, "IPCC 2006: 2.02 kgCO2/m³"))
            }
            "fleet_gasoline_liter" | "gasoline_liter" | "bensin_liter" => {
                Some((2.31, "IPCC 2006: 2.31 kgCO2/L"))
            }
            ...
            // Scope 3
            "business_travel_km" | "perjalanan_dinas_km" | "guest_flights_km" | "flights_km" => {
                Some((0.255, "DEFRA 2023: 0.255 kgCO2/pax-km (flight economy)"))
            }
            "commuting_km" | "komuter_km" => Some((0.17, "Motorcycle avg: 0.17 kgCO2/km")),
            "waste_landfill_ton" | "waste_ton" | "limbah_ton" => {
                Some((1200.0, "IPCC default: 1.2 tCO2e/ton mixed waste"))
            }
            ...
            "procurement_idr" | "purchased_goods_idr" => {
                Some((0.0000001, "Proxy: 100 gCO2e/M IDR"))
            }
    ...
    for s in &scope1 {
        match get_ef(&s.source, &s.unit) {
            ...
            None => errors.push(format!(
                "Scope 1: '{}' ({}) — faktor emisi tidak ditemukan",
                s.source, s.unit
            )),
        }
    }
    for s in &scope2 {
        match get_ef(&s.source, &s.unit) {
            ...
            None => errors.push(format!(
                "Scope 2: '{}' ({}) — faktor emisi tidak ditemukan",
                s.source, s.unit
            )),
        }
    }
    for s in &scope3 {
        match get_ef(&s.source, &s.unit) {
            ...
            None => errors.push(format!(
                "Scope 3: '{}' ({}) — faktor emisi tidak ditemukan",
                s.source, s.unit
            )),
        }
    }
    ...
    // Scope 1
    let s1_pct = if grand_total > 0.0 {
        s1_total / grand_total * 100.0
    } else {
        0.0
    };
    ...
    // Scope 2
    let s2_pct = if grand_total > 0.0 {
        s2_total / grand_total * 100.0
    } else {
        0.0
    };
    out.push_str(&format!(
        "SCOPE 2 — Emisi Tidak Langsung Energi ({:.1}%)\n",
        s2_pct
    ));
    ...
    // Scope 3
    let s3_pct = if grand_total > 0.0 {
        s3_total / grand_total * 100.0
    } else {
        0.0
    };
    out.push_str(&format!(
        "SCOPE 3 — Emisi Tidak Langsung Lainnya ({:.1}%)\n",
        s3_pct
    ));
    ...
    out.push_str(&format!(
        "  Per karyawan (asumsi 100): {:.2} tCO2e/orang\n",
        grand_total / 100.0
    ));
    out.push_str(&format!(
        "  Per M IDR revenue (asumsi 10B): {:.2} tCO2e/M IDR\n\n",
        grand_total / 10000.0
    ));
