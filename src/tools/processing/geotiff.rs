...
pub fn info(path: &str) -> String {
    match Command::new("gdalinfo").arg(path).arg("-json").output() {
        Ok(o) => {
            ...
            if out.is_empty() { format!("ERROR: gdalinfo gagal untuk {}", path) }
            else { format!("=== GeoTIFF Info ===\n{}", &out[..out.len().min(4000)]) }
        }
...
pub fn crop(input: &str, output: &str, bbox: &str) -> String {
    ...
    if parts.len() != 4 { return "ERROR: bbox harus 4 angka: minlon minlat maxlon maxlat".into(); }
    ...
    match Command::new("gdalwarp")
        ...
        Ok(o) => {
            ...
            if o.status.success() { format!("SUCCESS: Crop selesai → {}", output) }
...
pub fn reproject(input: &str, output: &str, target_crs: &str) -> String {
    match Command::new("gdalwarp")
        ...
        .output() {
        Ok(o) => {
            if o.status.success() { format!("SUCCESS: Reproject ke {} → {}", target_crs, output) }
            else { format!("ERROR: {}", String::from_utf8_lossy(&o.stderr)) }
        }
