...
pub fn gumbel(data: &[f64], return_period: f64) -> String {
    if data.len() < 10 {
        return "ERROR [E105]: Minimum 10 tahun data untuk analisis frekuensi.".into();
    }
    if return_period <= 1.0 {
        return "ERROR [E102]: Parameter harus > 1.".into();
    }
    ...
    let k = -(6.0_f64.sqrt() / std::f64::consts::PI)
        * (0.5772 + (return_period / (return_period - 1.0)).ln().ln());
...
pub fn log_pearson_iii(data: &[f64], return_period: f64) -> String {
    if data.len() < 10 {
        return "ERROR [E105]: Minimum 10 tahun data.".into();
    }
    if return_period <= 1.0 {
        return "ERROR [E102]: Parameter harus > 1.".into();
    }
    if data.iter().any(|&x| x <= 0.0) {
        return "ERROR [E102]: Parameter harus > 0 untuk Log-Pearson III.".into();
    }
