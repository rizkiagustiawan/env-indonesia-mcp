...
pub fn calculate(materials: &[(String, f64, f64, f64)]) -> String {
    ...
    if materials.is_empty() { return "ERROR: Masukkan minimal 1 material.".into(); }
    ...
    let cn_ratio = if total_n > 0.0 { total_c / total_n } else { 999.0 };

    let mut out = String::from("=== Composting C/N Ratio ===\n\n");
    for (name, mass, c, n) in materials {
        out.push_str(&format!("  {}: {:.0} kg (C={:.1}%, N={:.1}%)\n", name, mass, c, n));
    }
    out.push_str(&format!("\nTotal: {:.0} kg\nC total: {:.1} kg\nN total: {:.2} kg\nC/N ratio: {:.1}\n\n", total_mass, total_c, total_n, cn_ratio));

    let status = if cn_ratio < 20.0 { "Terlalu rendah — tambah bahan kaya karbon (serbuk gergaji, jerami)" }
    else if cn_ratio <= 35.0 { "OPTIMAL (25-35:1) — proses komposting efisien" }
    else { "Terlalu tinggi — tambah bahan kaya nitrogen (kotoran ayam, daun hijau)" };
