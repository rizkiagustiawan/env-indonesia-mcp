...
pub fn calculate(species_counts: &[u64]) -> String {
    if species_counts.is_empty() {
        return "ERROR: Masukkan jumlah individu per spesies.".into();
    }

    let total: u64 = species_counts.iter().sum();
    if total == 0 {
        return "ERROR: Total individu = 0.".into();
    }
    ...
    // Shannon-Wiener
    let shannon: f64 = species_counts
        .iter()
        .filter(|&&n| n > 0)
        .map(|&n| {
    ...
    let kat_h = if shannon < 1.0 {
        "Rendah (ekosistem tertekan)"
    } else if shannon < 2.0 {
        "Sedang"
    } else if shannon < 3.0 {
        "Tinggi (ekosistem stabil)"
    } else {
        "Sangat Tinggi (ekosistem pristine)"
    };
