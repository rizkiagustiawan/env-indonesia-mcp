...
pub fn calculate(predicted_json: &str, actual_json: &str) -> String {
    ...
    if predicted.len() != actual.len() {
        return format!("ERROR: predicted ({}) and actual ({}) arrays must have same length", predicted.len(), actual.len());
    }
    ...
    for p in &predicted { classes.insert(p.clone()); }
    for a in &actual { classes.insert(a.clone()); }
    let class_list: Vec<String> = classes.into_iter().collect();
    let nc = class_list.len();

    // Build class index map
    let class_idx: BTreeMap<&str, usize> = class_list.iter().enumerate().map(|(i, c)| (c.as_str(), i)).collect();
    ...
    // Cohen's Kappa
    let pe: f64 = (0..nc).map(|i| row_totals[i] as f64 * col_totals[i] as f64).sum::<f64>() / (total as f64 * total as f64);
    let kappa = (oa - pe) / (1.0 - pe);

    // Producer's and User's accuracy per class
    let producer_acc: Vec<f64> = (0..nc).map(|i| {
        if row_totals[i] > 0 { matrix[i][i] as f64 / row_totals[i] as f64 } else { 0.0 }
    }).collect();
    let user_acc: Vec<f64> = (0..nc).map(|j| {
        if col_totals[j] > 0 { matrix[j][j] as f64 / col_totals[j] as f64 } else { 0.0 }
    }).collect();
    ...
    for i in 0..nc {
        out.push_str(&format!("{:<15}{:>14.1}%{:>14.1}%\n",
    ...
    out.push_str(&format!("Overall Accuracy >= 85%: {} ({})\n",
        if oa_pass { "LULUS" } else { "TIDAK LULUS" },
        format!("{:.1}%", oa * 100.0)));
    out.push_str(&format!("Kappa >= 0.75          : {} ({})\n",
