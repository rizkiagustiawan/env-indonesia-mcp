...
pub fn score(impacts: &[(String, String, i32, u32)]) -> String {
    ...
    if impacts.is_empty() {
        return "ERROR: Masukkan minimal 1 dampak (kegiatan, komponen, magnitude, importance)."
            .into();
    }
    ...
    for (kegiatan, komponen, mag, imp) in impacts {
        if *mag < -10 || *mag > 10 {
            out.push_str(&format!(
                "⚠️ Magnitude {} di luar rentang (-10 s.d. +10)\n",
                mag
            ));
            continue;
        }
        if *imp < 1 || *imp > 10 {
            out.push_str(&format!("⚠️ Importance {} di luar rentang (1-10)\n", imp));
            continue;
        }
        ...
        out.push_str(&format!(
            "{:<30} {:<25} {:>5} {:>5} {:>5} {}\n",
            &kegiatan[..kegiatan.len().min(29)],
            &komponen[..komponen.len().min(24)],
            mag,
            imp,
            score,
            indicator
