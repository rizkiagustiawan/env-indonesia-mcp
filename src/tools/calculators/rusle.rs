...
pub fn calculate(r_input: Option<f64>, rain_mm_yr: Option<f64>, k: f64, ls: f64, c: f64, p: f64) -> String {
    ...
    if ls < 0.0 {
        return format!("ERROR [E102]: Parameter tidak boleh negatif. {}", ls);
    }
    if c < 0.0 || c > 1.0 {
        return format!("ERROR: C-cover ({}) harus 0-1.", c);
    }
    if p < 0.0 || p > 1.0 {
        return format!("ERROR: P-practice ({}) harus 0-1.", p);
    }
...
pub fn compute_ls(slope_pct: f64, length_m: f64) -> String {
    ...
    let ls =
        (length_m / 22.13_f64).powf(m) * (65.41 * theta.sin().powi(2) + 4.56 * theta.sin() + 0.065);
    format!(
        "=== LS Factor ===\nSlope: {:.1}%\nLength: {:.1} m\nm-exponent: {:.1}\nLS = {:.3}\n",
