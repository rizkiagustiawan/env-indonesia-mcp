...
pub fn calculate(
    secchi_depth_m: Option<f64>,
    chlorophyll_ugl: Option<f64>,
    total_phosphorus_ugl: Option<f64>,
) -> String {
    ...
    if let Some(chl) = chlorophyll_ugl {
        ...
        out.push_str(&format!(
            "Chlorophyll-a = {:.2} µg/L → TSI(Chl) = {:.1}\n",
            chl, tsi
        ));
    }
    if let Some(tp) = total_phosphorus_ugl {
        ...
        out.push_str(&format!(
            "Total Phosphorus = {:.2} µg/L → TSI(TP) = {:.1}\n",
            tp, tsi
        ));
    }

    if tsi_values.is_empty() {
        return "ERROR: Masukkan minimal 1 parameter (secchi/chlorophyll/phosphorus).".into();
    }

    let avg_tsi: f64 = tsi_values.iter().map(|(_, v)| v).sum::<f64>() / tsi_values.len() as f64;
    let cat = if avg_tsi < 40.0 {
        "Oligotrofik (bersih)"
    } else if avg_tsi < 50.0 {
        "Mesotrofik (sedang)"
    } else if avg_tsi < 70.0 {
        "Eutrofik (subur berlebih)"
    } else {
        "Hipereutrofik (sangat tercemar nutrisi)"
    };
