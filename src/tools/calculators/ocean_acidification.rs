...
pub fn calculate(ph: f64, pco2_uatm: f64, temp_c: f64, salinity_psu: f64) -> String {
    if ph < 7.0 || ph > 9.0 {
        return format!("ERROR: pH laut {} di luar rentang normal (7.0-9.0).", ph);
    }
    if pco2_uatm <= 0.0 {
    ...
    let status = if omega_arag > 3.5 {
        "Sehat (supersaturated)"
    } else if omega_arag > 1.0 {
        "Marginal (coral stress mulai)"
    } else {
        "KRITIS (undersaturated — coral dissolution)"
    };
