...
pub fn calculate(roof_area_m2: f64, rainfall_mm: f64, runoff_coeff: f64, demand_liters_day: f64) -> String {
    ...
    let days_supply = if demand_liters_day > 0.0 { supply_liters / demand_liters_day } else { 0.0 };
    ...
    out.push_str(&format!("Supply = {:.0} liter/bulan\nCukup untuk {:.0} hari\nRekomendasi tangki: {:.0} liter\n", supply_liters, days_supply, supply_liters * 0.8));
    out
}
