...
pub fn route(inflow_hydrograph: &[(f64, f64)], k_hours: f64, x: f64, dt_hours: f64) -> String {
    ...
    if inflow_hydrograph.len() < 2 {
        return "ERROR: Minimal 2 titik data inflow diperlukan.".into();
    }
    if k_hours <= 0.0 {
        return "ERROR [E102]: Parameter harus > 0.".into();
    }
    if x < 0.0 || x > 0.5 {
        return "ERROR: x harus antara 0 dan 0.5.".into();
    }
    if dt_hours <= 0.0 {
        return "ERROR [E102]: Parameter harus > 0.".into();
    }
    ...
    if dt_hours < lower || dt_hours > upper {
        out.push_str(&format!(
            "⚠️ PERINGATAN: Δt = {:.2} jam di luar rentang stabil [{:.2}, {:.2}] jam\n\n",
    ...
    out.push_str(&format!(
        "  C₀ = (-Kx + 0.5Δt) / (K-Kx+0.5Δt) = {:.4}\n",
        c0
    ));
    out.push_str(&format!("  C₁ = (Kx + 0.5Δt) / (K-Kx+0.5Δt) = {:.4}\n", c1));
    out.push_str(&format!(
        "  C₂ = (K-Kx - 0.5Δt) / (K-Kx+0.5Δt) = {:.4}\n",
        c2
    ));
    ...
    // Find peaks
    let peak_in = inflow_hydrograph
        .iter()
        .max_by(|a, b| a.1.partial_cmp(&b.1).unwrap())
        .unwrap();
    let peak_out = outflow
        .iter()
        .max_by(|a, b| a.1.partial_cmp(&b.1).unwrap())
        .unwrap();
