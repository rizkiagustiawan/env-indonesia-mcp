...
pub fn calculate(
    ...
    let mapped_areas: Vec<f64> = match serde_json::from_str(mapped_areas_json) {
        Ok(v) => v,
        Err(e) => return format!("ERROR [E103]: mapped_areas JSON: {}", e),
    };
    let matrix: Vec<Vec<u32>> = match serde_json::from_str(confusion_matrix_json) {
        Ok(v) => v,
        Err(e) => return format!("ERROR [E103]: confusion_matrix JSON: {}", e),
    };
    let class_names: Vec<String> = serde_json::from_str(class_names_json).unwrap_or_else(|_| {
        (0..mapped_areas.len())
    ...
    if a_total <= 0.0 {
        return "ERROR [E102]: Total mapped area harus > 0".into();
    }
    ...
    let n_i: Vec<f64> = matrix
        .iter()
        .map(|row| row.iter().sum::<u32>() as f64)
        .collect();
    ...
    for i in 0..q {
        if n_i[i] < 2.0 {
            return format!(
                "ERROR [E105]: Stratum {} ('{}') memiliki < 2 sampel. Minimum 2 per stratum.",
                i, class_names[i]
            );
        }
    }
    ...
    let p: Vec<Vec<f64>> = (0..q)
        .map(|i| {
            (0..q)
                .map(|j| w[i] * (matrix[i][j] as f64 / n_i[i]))
                .collect()
        })
        .collect();
    ...
    let col_sums: Vec<f64> = (0..q)
        .map(|j| (0..q).map(|i| p[i][j]).sum::<f64>())
        .collect();
    let producers_acc: Vec<(f64, f64)> = (0..q)
        .map(|j| {
            if col_sums[j] > 0.0 {
                (p[j][j] / col_sums[j], 0.0)
            }
            // simplified SE
            else {
                (0.0, 0.0)
    ...
    let area_estimates: Vec<(f64, f64, f64, f64)> = (0..q)
        .map(|j| {
    ...
    for name in &class_names {
        out.push_str(&format!("{:<12}", &name[..name.len().min(11)]));
    }
    ...
    out.push_str(&format!(
        "95% CI: [{:.2}%, {:.2}%]\n",
        (oa - z_score * se_oa).max(0.0) * 100.0,
        (oa + z_score * se_oa).min(1.0) * 100.0
    ));
