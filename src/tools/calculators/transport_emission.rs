...
pub fn calculate(fuel_type: &str, liters: f64) -> String {
    ...
    let (ef_kg_l, ncv_tj_gq, fuel_name) = match fuel_type.to_lowercase().as_str() {
        ...
        _ => return format!("Tipe bahan bakar '{}' tidak valid. Gunakan: bensin, solar, atau avtur.", fuel_type),
    };
    ...
    out.push_str(&format!("Bahan Bakar: {}\nVolume: {:.2} Liter\n\n", fuel_name, liters));
    out.push_str(&format!("Faktor Emisi (Default): {:.2} kg CO₂/Liter\nNet Calorific Value: {:.1} TJ/Gg\n\n", ef_kg_l, ncv_tj_gq));
    
    out.push_str(&format!("Total Emisi CO₂:\n  = {:.2} kg CO₂\n  = {:.4} ton CO₂\n", co2_kg, co2_ton));
