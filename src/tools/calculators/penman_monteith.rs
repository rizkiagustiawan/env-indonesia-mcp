...
pub fn calculate(t_mean: f64, rh_mean: f64, u2: f64, rn: f64) -> String {
    ...
    if rh_mean < 0.0 || rh_mean > 100.0 {
        return format!("ERROR: RH ({}) harus 0-100%.", rh_mean);
    }
    ...
    if et0 < 0.0 {
        out.push_str("⚠️ ET0 negatif — kemungkinan kondisi malam / embun. Set ET0 = 0.\n");
    } else if et0 > 15.0 {
        out.push_str("⚠️ ET0 > 15 mm/hari — sangat tinggi, cek input Rn dan u2.\n");
    }

    let kategori = if et0 < 3.0 {
        "Rendah (dataran tinggi/humid)"
    } else if et0 < 5.5 {
        "Sedang (tropis humid)"
    } else if et0 < 8.0 {
        "Tinggi (tropis kering)"
    } else {
        "Sangat Tinggi"
    };
