...
pub fn calculate(
    ...
    let (k1_eff, k2_eff) = match temp_c {
        Some(t) => {
            ...
            out.push_str(&format!(
                "  k1(20°C) = {:.4} → k1(T) = {:.4} (θ=1.047, Metcalf & Eddy)\n",
                k1, k1_t
            ));
            out.push_str(&format!(
                "  k2(20°C) = {:.4} → k2(T) = {:.4} (θ=1.024, O'Connor & Dobbins)\n\n",
                k2, k2_t
            ));
            (k1_t, k2_t)
        }
    ...
    let dc = if tc > 0.0 {
        ((k1_eff * l0) / (k2_eff - k1_eff)) * ((-k1_eff * tc).exp() - (-k2_eff * tc).exp())
            + d0 * (-k2_eff * tc).exp()
    } else {
        -1.0
    };
    ...
    // Jarak kritis
    let xc = if tc > 0.0 && velocity_ms > 0.0 {
        tc * velocity_ms * 86400.0 / 1000.0
    } else {
        0.0
    }; // km
    ...
    let steps = 10;
    let max_dist = if distance_km > 0.0 {
        distance_km
    } else if tc > 0.0 {
        xc * 3.0
    } else {
    ...
    for i in 0..=steps {
        ...
        let t = if velocity_ms > 0.0 {
            x * 1000.0 / (velocity_ms * 86400.0)
        } else {
            0.0
        };
        let d = ((k1_eff * l0) / (k2_eff - k1_eff)) * ((-k1_eff * t).exp() - (-k2_eff * t).exp())
            + d0 * (-k2_eff * t).exp();
        out.push_str(&format!(
            "  {:.1} km | {:.2} hari | {:.2} mg/L\n",
            x,
