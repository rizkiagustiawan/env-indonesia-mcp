        ...
        Ok(resp) => match resp.text().await {
            Ok(body) => {
                ...
                if let Ok(v) = serde_json::from_str::<serde_json::Value>(&body) {
                                    ...
                                    if let Some(hours) = day.as_array() {
                                        for h in hours.iter().take(4) {
                                            ...
                                            let desc = h
                                                ...
                                                .unwrap_or("?");
                                            let temp =
                                                h.get("t").and_then(|t| t.as_f64()).unwrap_or(0.0);
                                            let humidity =
                                                h.get("hu").and_then(|h| h.as_f64()).unwrap_or(0.0);
                                            let wind_spd =
                                                h.get("ws").and_then(|w| w.as_f64()).unwrap_or(0.0);
                                            out.push_str(&format!(
                                                "  {} | {}°C | {}% RH | {:.0} km/h | {}\n",
                    ...
                    format!(
                        "Raw BMKG response (parse failed): {}",
