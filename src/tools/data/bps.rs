...
fn province_domain_id(province: &str) -> Option<(&'static str, &'static str)> {
    ...
    match () {
        ...
        _ if p.contains("sumatera utara") || p.contains("sumut") => Some(("1200", "Sumatera Utara")),
        _ if p.contains("sumatera barat") || p.contains("sumbar") => Some(("1300", "Sumatera Barat")),
        _ if p.contains("riau") && !p.contains("kepulauan") => Some(("1400", "Riau")),
        _ if p.contains("jambi") => Some(("1500", "Jambi")),
        _ if p.contains("sumatera selatan") || p.contains("sumsel") => Some(("1600", "Sumatera Selatan")),
        ...
        _ if p.contains("kepulauan riau") || p.contains("kepri") => Some(("2100", "Kepulauan Riau")),
        ...
        _ if p.contains("ntb") || p.contains("nusa tenggara barat") => Some(("5200", "Nusa Tenggara Barat")),
        _ if p.contains("ntt") || p.contains("nusa tenggara timur") => Some(("5300", "Nusa Tenggara Timur")),
        _ if p.contains("kalimantan barat") || p.contains("kalbar") => Some(("6100", "Kalimantan Barat")),
        _ if p.contains("kalimantan tengah") || p.contains("kalteng") => Some(("6200", "Kalimantan Tengah")),
        _ if p.contains("kalimantan selatan") || p.contains("kalsel") => Some(("6300", "Kalimantan Selatan")),
        _ if p.contains("kalimantan timur") || p.contains("kaltim") => Some(("6400", "Kalimantan Timur")),
        _ if p.contains("kalimantan utara") || p.contains("kaltara") => Some(("6500", "Kalimantan Utara")),
        _ if p.contains("sulawesi utara") || p.contains("sulut") => Some(("7100", "Sulawesi Utara")),
        _ if p.contains("sulawesi tengah") || p.contains("sulteng") => Some(("7200", "Sulawesi Tengah")),
        _ if p.contains("sulawesi selatan") || p.contains("sulsel") => Some(("7300", "Sulawesi Selatan")),
        _ if p.contains("sulawesi tenggara") || p.contains("sultra") => Some(("7400", "Sulawesi Tenggara")),
        _ if p.contains("gorontalo") => Some(("7500", "Gorontalo")),
        _ if p.contains("sulawesi barat") || p.contains("sulbar") => Some(("7600", "Sulawesi Barat")),
        ...
        _ if p.contains("papua") && !p.contains("barat") => Some(("9400", "Papua")),
        _ => None,
    }
}
...
pub async fn statistics(client: &Client, keyword: &str) -> String {
    ...
    let province_name = province_info.map(|(_, name)| name).unwrap_or("Indonesia (Nasional)");

    let api_key = std::env::var("BPS_API_KEY").unwrap_or_default();

    let mut out = format!("=== BPS Statistik Lingkungan — {} (Query: {}) ===\n\n", province_name, keyword);
    ...
    if !api_key.is_empty() {
                        ...
                        if v.get("status").and_then(|s| s.as_str()) == Some("OK") {
                            if let Some(data) = v.get("data").and_then(|d| d.as_array()) {
                                ...
                                for (i, d) in data.iter().take(5).enumerate() {
                                    let val = d.get("data_content").and_then(|v| v.as_str()).unwrap_or("-");
                                    let year = d.get("tahun").and_then(|y| y.as_str()).unwrap_or("?");
                                    if i > 0 { out.push_str(", "); }
        ...
        out.push_str("(BPS API tidak mengembalikan data yang relevan, menggunakan data fallback)\n\n");
    } else {
        out.push_str("(BPS_API_KEY tidak diset — menggunakan data fallback statis)\n\n");
    }

    // === FALLBACK: hardcoded data ===
    out.push_str(&format!("Data Referensi Utama (BPS 2023-2025, {}):\n", province_name));

    let l = keyword.to_lowercase();
    if l.contains("hutan") || l.contains("forest") {
        if province_info.is_some() && keyword_lower.contains("ntb") {
    ...
    out.push_str(&format!("Domain ID untuk {}: {}\n", province_name, domain_id));
    out
}
