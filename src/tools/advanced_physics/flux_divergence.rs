...
pub fn calculate_emissions(
    ...
    if parsed.is_err() {
        return "ERROR: Format grid_data_json tidak valid. Harap berikan array 2D konsentrasi.".into();
    }
    ...
    out.push_str(&format!("Angin: u={:.1} m/s, v={:.1} m/s\n", u_wind, v_wind));
    out.push_str(&format!("Siklus hidup polutan (τ): {:.1} jam\n\n", lifetime_hours));
    
    out.push_str("HASIL ANALISIS:\n");
    out.push_str(&format!("  Emisi Tertinggi terdeteksi di grid [{}, {}]\n", max_loc.0, max_loc.1));
    out.push_str(&format!("  Nilai Flux: {:.4} (unit/m²/s)\n\n", max_emission));
