...
pub fn screen(material: &str, activity_bq_g: f64) -> String {
    ...
    let materials = [
        NormMaterial { name: "tin_slag",      display: "Terak Timah (Tin Slag)",       typical_range_bq_g: (0.1, 5.0),    primary_nuclide: "Ra-228, Th-228", source: "Peleburan timah Bangka-Belitung" },
        NormMaterial { name: "monazite",      display: "Monazit",                      typical_range_bq_g: (10.0, 200.0),  primary_nuclide: "Th-232, U-238",  source: "Penambangan pasir mineral" },
        NormMaterial { name: "zircon_sand",   display: "Pasir Zirkon",                 typical_range_bq_g: (1.0, 10.0),    primary_nuclide: "U-238, Th-232",  source: "Penambangan pasir mineral" },
        NormMaterial { name: "coal_ash",      display: "Abu Batubara (Fly/Bottom Ash)", typical_range_bq_g: (0.05, 1.0),   primary_nuclide: "Ra-226, K-40",   source: "PLTU batubara" },
        NormMaterial { name: "phosphogypsum", display: "Fosfogipsum",                  typical_range_bq_g: (0.1, 2.0),     primary_nuclide: "Ra-226",         source: "Industri pupuk fosfat" },
        NormMaterial { name: "bauxite_residue", display: "Residu Bauksit (Red Mud)",   typical_range_bq_g: (0.1, 3.0),     primary_nuclide: "Ra-226, Th-232", source: "Pengolahan aluminium" },
        NormMaterial { name: "oil_scale",     display: "Kerak Minyak (Oil Pipe Scale)", typical_range_bq_g: (0.5, 50.0),   primary_nuclide: "Ra-226, Ra-228", source: "Industri minyak & gas" },
        NormMaterial { name: "fertilizer",    display: "Pupuk Fosfat",                 typical_range_bq_g: (0.01, 0.5),    primary_nuclide: "U-238, Ra-226",  source: "Industri pupuk" },
    ];
    ...
    if let Some(info) = mat_info {
        ...
        result.push_str(&format!("• Nuklida utama        : {}\n", info.primary_nuclide));
        result.push_str(&format!("• Rentang tipikal      : {:.2} – {:.2} Bq/g\n", info.typical_range_bq_g.0, info.typical_range_bq_g.1));
    } else {
        result.push_str(&format!("• Material             : {} (tidak ada data tipikal)\n", material));
    }
    result.push_str(&format!("• Aktivitas terukur    : {:.4} Bq/g\n\n", activity_bq_g));

    result.push_str("TINGKAT PEMBEBASAN (PerKa BAPETEN No. 4/2013):\n");
    result.push_str(&format!("• Umum (general)       : {:.1} Bq/g  {}\n",
        clearance_general,
        if !above_general { "✓ LULUS" } else { "✗ MELEBIHI" }
    ));
    result.push_str(&format!("• Curah/konstruksi     : {:.1} Bq/g {}\n\n",
        clearance_bulk,
        if !above_bulk { "✓ LULUS" } else { "✗ MELEBIHI" }
    ));

    result.push_str(&format!("KLASIFIKASI: {}\n\n", classification));

    result.push_str("ESTIMASI DOSIS (pekerja, 2000 jam/tahun):\n");
    result.push_str(&format!("• Dosis eksternal      : {:.4} mSv/tahun\n", external_dose));
    result.push_str(&format!("• Dosis inhalasi debu  : {:.4} mSv/tahun\n", inhalation_dose));
    result.push_str(&format!("• Total estimasi       : {:.4} mSv/tahun\n", total_annual_dose));
    result.push_str(&format!("• Batas pekerja        : 20 mSv/tahun {}\n\n",
        if total_annual_dose <= 20.0 { "✓" } else { "✗ MELEBIHI" }
    ));
    ...
    for m in &materials {
        result.push_str(&format!("  • {:30} : {:.2}–{:.2} Bq/g ({})\n",
