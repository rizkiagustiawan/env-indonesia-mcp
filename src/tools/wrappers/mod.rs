...
pub async fn trigger_esg_audit(client: &Client, site_id: &str) -> String {
        ...
        .await
    {
        ...
        Err(e) => format!(
            "Service offline or error: {}. Pastikan GeoESG-Final berjalan di port {}.",
            e, GEO_ESG_PORT
        ),
    }
}
...
pub async fn predict_flood(client: &Client, lat: f64, lon: f64) -> String {
    let url = format!(
        "http://localhost:{}/predict/at?lat={}&lon={}",
        FLOOD_AI_PORT, lat, lon
    );
    match client.get(&url).send().await {
        ...
        Err(e) => format!(
            "Service offline or error: {}. Pastikan flood-ai berjalan di port {}.",
            e, FLOOD_AI_PORT
        ),
    }
}
...
pub async fn get_methane_plumes(client: &Client) -> String {
    ...
    match client.get(&url).send().await {
        ...
        Err(e) => format!(
            "Service offline or error: {}. Pastikan methane-monitor berjalan di port {}.",
            e, METHANE_PORT
        ),
    }
}
...
pub async fn get_groundwater_status(client: &Client) -> String {
    ...
    match client.get(&url).send().await {
        ...
        Err(e) => format!(
            "Service offline or error: {}. Pastikan groundwater-monitor berjalan di port {}.",
            e, GROUNDWATER_PORT
        ),
    }
}
...
pub async fn get_air_quality(client: &Client) -> String {
    ...
    match client.get(&url).send().await {
        ...
        Err(e) => format!(
            "Service offline or error: {}. Pastikan airtestingquality berjalan di port {}.",
            e, AIR_QUALITY_PORT
        ),
    }
}
