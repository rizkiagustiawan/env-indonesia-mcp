...
pub fn calculate(data_json: &str) -> String {
    ...
    if params.is_empty() {
        return "ERROR: Masukkan minimal 1 parameter.".into();
    }
    ...
    for p in params {
        ...
        if samples.is_none() || samples.unwrap().is_empty() {
            continue;
        }
        ...
        if max_val > limit {
            param_score += w_max_min;
        }
        if min_val > limit {
            param_score += w_max_min;
        } // Asumsi baku mutu maks. Untuk parameter minimum (spt DO), logika di-handle pre-processing.
        if mean_val > limit {
            param_score += w_mean;
        }
        ...
        out.push_str(&format!(
            "{:<10} ({} smpl) | Max:{:.1} Min:{:.1} Avg:{:.1} BM:{:.1} -> Skor: {}\n",
    ...
    let status = if total_score == 0 {
        "Kelas A: Memenuhi Baku Mutu (Baik Sekali)"
    } else if total_score >= -10 {
        "Kelas B: Cemar Ringan"
    } else if total_score >= -30 {
        "Kelas C: Cemar Sedang"
    } else {
        "Kelas D: Cemar Berat"
    };
