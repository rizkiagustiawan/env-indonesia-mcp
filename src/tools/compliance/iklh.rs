...
pub fn calculate(ika: f64, iku: f64, iktl: f64) -> String {
    ...
    if ika < 0.0 || ika > 100.0 {
        return format!("ERROR: IKA ({:.1}) harus 0-100.", ika);
    }
    if iku < 0.0 || iku > 100.0 {
        return format!("ERROR: IKU ({:.1}) harus 0-100.", iku);
    }
    if iktl < 0.0 || iktl > 100.0 {
        return format!("ERROR: IKTL ({:.1}) harus 0-100.", iktl);
    }
    ...
    out.push_str(&format!(
        "IKLH = ({:.2}×0.30) + ({:.2}×0.30) + ({:.2}×0.40) = {:.2}\n\n",
        ika, iku, iktl, iklh
    ));

    let kategori = if iklh >= 80.0 {
        "SANGAT BAIK"
    } else if iklh >= 70.0 {
        "BAIK"
    } else if iklh >= 60.0 {
