...
pub fn classify(waste_type: &str) -> String {
    ...
    let (kategori, kode, penjelasan, penanganan) = if w.contains("aki")
        || w.contains("baterai")
        || w.contains("battery")
    {
        (
            "KATEGORI 1 — B3 AKUT",
            "A101d",
            "Baterai/aki bekas mengandung Pb (timbal) dan H2SO4.",
            "Wajib dikumpulkan oleh pengumpul berizin. Dilarang dibuang ke TPA. Manifes B3 wajib.",
        )
    } else if w.contains("oli") || w.contains("pelumas") || w.contains("lubricant") {
        (
            "KATEGORI 1 — B3",
            "B105d",
            "Oli/pelumas bekas mengandung logam berat dan PAH.",
            "Wajib diolah di pengolah berizin (re-refining/insinerasi). Penyimpanan max 90 hari.",
        )
    } else if w.contains("pestisida") || w.contains("insektisida") || w.contains("herbisida") {
        (
            "KATEGORI 1 — B3 AKUT",
            "A103d",
            "Pestisida kadaluarsa atau sisa kemasan.",
            "Wajib insinerasi pada suhu > 1200°C. Sangat berbahaya bagi ekosistem akuatik.",
        )
    } else if w.contains("merkuri") || w.contains("mercury") || w.contains("hg") {
        (
            "KATEGORI 1 — B3 AKUT",
            "A104d",
            "Limbah mengandung merkuri (Hg). Sangat toksik dan bioakumulatif.",
            "Stabilisasi/solidifikasi + penimbunan akhir di TPS LB3. Minamata Convention.",
        )
    } else if w.contains("medis") || w.contains("rumah sakit") || w.contains("medical") {
        (
            "KATEGORI 1 — B3 INFEKSIUS",
            "A337-1",
            "Limbah medis infeksius (jarum, darah, organ, obat kadaluarsa).",
            "Wajib autoklaf/insinerasi. Dilarang dicampur sampah domestik. PermenLHK 56/2015.",
        )
    } else if w.contains("sludge") || w.contains("lumpur") {
        (
            "KATEGORI 2 — B3",
            "B106d",
            "Lumpur IPAL mengandung logam berat dan zat organik persisten.",
            "Uji TCLP wajib. Jika lolos → non-B3. Jika tidak → pengolahan khusus.",
        )
        ...
        (
            "KATEGORI 1 — B3",
            "A108d",
            "Limbah elektronik (PCB, CRT, chip) mengandung Pb, Hg, Cd, BFR.",
            "Wajib dikumpulkan pengumpul berizin. Extended Producer Responsibility (EPR).",
        )
    } else {
