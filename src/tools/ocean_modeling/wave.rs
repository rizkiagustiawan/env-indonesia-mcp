...
pub fn jonswap(wind_speed_ms: f64, fetch_m: f64, depth_m: f64) -> String {
    ...
    out.push_str(&format!("INPUT:\n  Wind speed = {:.1} m/s\n  Fetch = {:.0} m ({:.1} km)\n  Depth = {:.1} m\n\n", u, fetch_m, fetch_m/1000.0, depth_m));
...
pub fn coral_bleaching_dhw(sst_weekly: &[f64], sst_max_monthly_mean: f64) -> String {
    if sst_weekly.is_empty() { return "ERROR: Minimal 1 data SST mingguan.".into(); }
    ...
    let risk = if dhw < 4.0 { "Watch — Waspada, bleaching belum terjadi" }
    else if dhw < 8.0 { "Warning — Bleaching kemungkinan besar terjadi" }
    else { "Alert Level 2 — Bleaching parah dan kematian coral" };

    let mut out = String::from("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n  Coral Bleaching DHW\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
    out.push_str("Ref: NOAA Coral Reef Watch\n\n");
    out.push_str(&format!("SST Max Monthly Mean = {:.1}°C\nData SST: {} minggu\n\n", sst_max_monthly_mean, sst_weekly.len()));
