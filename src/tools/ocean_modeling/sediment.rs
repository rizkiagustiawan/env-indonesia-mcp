...
pub fn cerc_transport(hs_m: f64, wave_angle_deg: f64, _beach_slope_deg: f64) -> String {
    ...
    if wave_angle_deg.abs() > 90.0 {
        return "ERROR: Wave angle harus -90 to 90 deg.".into();
    }
    ...
    let direction = if wave_angle_deg > 0.0 {
        "ke KANAN (melihat ke laut)"
    } else {
        "ke KIRI (melihat ke laut)"
    };
