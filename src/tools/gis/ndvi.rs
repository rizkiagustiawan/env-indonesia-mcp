pub fn compute(nir: f64, red: f64) -> String {
    let ndvi = if (nir + red).abs() < 1e-10 { 0.0 } else { (nir - red) / (nir + red) };
    ...
    let health = if ndvi > 0.6 { "Healthy" } else if ndvi > 0.3 { "Moderate" } else if ndvi > 0.1 { "Stressed" } else { "Non-vegetated" };
