pub fn quality(green: f64, red: f64, nir: f64, blue: Option<f64>) -> String {
    let ndwi = if (green + nir).abs() < 1e-10 {
        0.0
    } else {
        (green - nir) / (green + nir)
    };
    ...
    let turbidity_ratio = if green.abs() < 1e-10 {
        0.0
    } else {
        red / green
    };
    let turbidity_class = if turbidity_ratio < 0.5 {
        "Clear"
    } else if turbidity_ratio < 1.0 {
        "Moderate"
    } else {
        "Turbid"
    };
    ...
    let chl_class = if chl_a_proxy < 1.0 {
        "Low (oligotrophic)"
    } else if chl_a_proxy < 2.0 {
