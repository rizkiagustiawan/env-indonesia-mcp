...
pub async fn search(client: &Client, lat: f64, lon: f64, days: u32) -> String {
    ...
    let start_date = (chrono::Utc::now() - chrono::Duration::days(days as i64)).format("%Y-%m-%d").to_string();
            ...
            Ok(v) => {
                ...
                if let Some(features) = v.get("features").and_then(|f| f.as_array()) {
                    for (i, f) in features.iter().enumerate() {
                        ...
                        let dt = f.get("properties").and_then(|p| p.get("datetime")).and_then(|d| d.as_str()).unwrap_or("?");
                        let cloud = f.get("properties").and_then(|p| p.get("eo:cloud_cover")).and_then(|c| c.as_f64());
                        out.push_str(&format!("{}. {} | {} | Cloud: {:.0}%\n", i+1, id, &dt[..10.min(dt.len())], cloud.unwrap_or(0.0)));
                    }
                }
