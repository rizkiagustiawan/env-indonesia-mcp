...
pub async fn search(client: &Client, collection: &str, limit: u32, bbox_opt: Option<String>) -> String {
    ...
    let bbox_str = bbox_opt.unwrap_or_else(|| "-11.0,95.0,6.0,141.0".to_string());
    let coords: Vec<f64> = bbox_str.split(',').filter_map(|s| s.trim().parse::<f64>().ok()).collect();
    let (s, w, n, e) = if coords.len() == 4 {
        (coords[0], coords[1], coords[2], coords[3])
    } else {
        (-11.0, 95.0, 6.0, 141.0) // Fallback Indonesia
    };
    ...
    out.push_str(&format!("Bounding box: {:.2}, {:.2}, {:.2}, {:.2}\n\n", s, w, n, e));
            ...
            Ok(v) => {
                if let Some(results) = v.get("value").and_then(|r| r.as_array()) {
                    ...
                    for (i, r) in results.iter().enumerate() {
                        ...
                        let date = r.get("ContentDate").and_then(|d| d.get("Start")).and_then(|s| s.as_str()).unwrap_or("?");
                        let size = r.get("ContentLength").and_then(|s| s.as_u64()).unwrap_or(0);
                        let id = r.get("Id").and_then(|i| i.as_str()).unwrap_or("?");
                        
                        out.push_str(&format!("{}. {}\n   TANGGAL: {}\n   SIZE: {:.1} MB\n   ID: {}\n", 
                            i+1, name, date, size as f64 / 1_048_576.0, id));
                        ...
                        if collection_id == "SENTINEL-2" {
                            ...
                            out.push_str(&format!("   🔗 PREVIEW / DOWNLOAD LANGSUNG: {}\n", viewer_link));
                        }
                        out.push_str("\n");
                    }
                } else {
