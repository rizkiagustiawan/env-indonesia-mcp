...
pub fn calculate(
    ...
    if transmissivity_m2s <= 0.0 {
        return "ERROR [E102]: Parameter harus > 0.".into();
    }
    if storativity <= 0.0 || storativity >= 1.0 {
        return "ERROR: Storativity (S) harus antara 0 dan 1.".into();
    }
    if r_m <= 0.0 {
        return "ERROR [E102]: Parameter harus > 0.".into();
    }
    ...
    let s_cj = if cj_valid {
        (2.3 * q_m3s) / (4.0 * pi * transmissivity_m2s)
            * (2.25 * transmissivity_m2s * t_s / (r_m * r_m * storativity)).log10()
    } else {
        0.0
    };
    ...
    if t_hours >= 1.0 {
        out.push_str(&format!(" ({:.1} jam)", t_hours));
    }
    if t_days >= 1.0 {
        out.push_str(&format!(" ({:.1} hari)", t_days));
    }
    out.push_str("\n\n");
    ...
    if cj_valid {
        out.push_str(&format!(
            "Penurunan (Cooper-Jacob): s = {:.3} m (u < 0.01, aproksimasi valid)\n",
            s_cj
        ));
    } else {
        out.push_str(&format!(
            "Cooper-Jacob: u = {:.4e} ≥ 0.01 — aproksimasi TIDAK valid, gunakan Theis.\n",
            u
        ));
    }
