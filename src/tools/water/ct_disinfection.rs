...
pub fn calculate(disinfectant: &str, concentration_mgl: f64, contact_time_min: f64, target_pathogen: &str) -> String {
    ...
    let (ct_table, unit_label): (Option<[f64; 4]>, &str) = match (disinfectant_lower.as_str(), pathogen_lower.as_str()) {
    ...
    if disinfectant_lower == "uv" {
        out.push_str(&format!("Dosis UV = {:.1} mJ/cm² (konsentrasi × waktu sebagai dosis)\n\n", ct_achieved));
    } else {
        out.push_str(&format!("CT tercapai = {:.2} × {:.1} = {:.2} mg·min/L\n\n", concentration_mgl, contact_time_min, ct_achieved));
    }

    match ct_table {
        Some(table) => {
            out.push_str(&format!("Tabel CT untuk {} terhadap {} (pH 7, 25°C):\n", disinfectant, target_pathogen));
            let log_labels = ["1-log", "2-log", "3-log", "4-log"];
            let pct_labels = ["90%", "99%", "99.9%", "99.99%"];
            for i in 0..4 {
                let status = if ct_achieved >= table[i] { "✅" } else { "❌" };
                out.push_str(&format!("  {} ({} removal): CT = {:.2} {} {}\n",
            ...
            out.push_str(&format!("Status kepatuhan (min 3-log): {}\n",
                if compliant { "✅ MEMENUHI" } else { "❌ TIDAK MEMENUHI — tingkatkan C atau t" }));
        }
        None => {
            out.push_str(&format!("ERROR: Kombinasi disinfektan '{}' dan patogen '{}' tidak tersedia.\n", disinfectant, target_pathogen));
