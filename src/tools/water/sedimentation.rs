...
pub fn design(q_m3d: f64, tank_type: &str, tank_shape: &str) -> String {
    ...
    let (overflow_rate, detention_hr, depth_m, weir_max) = match type_lower.as_str() {
        ...
        _ => {
            return format!(
                "ERROR: Tipe bak '{}' tidak dikenali. Pilihan: primary/primer, secondary/sekunder.",
                tank_type
            )
        }
    };
    ...
    if !["rectangular", "persegi", "circular", "bundar"].contains(&shape_lower.as_str()) {
        return format!(
            "ERROR: Bentuk bak '{}' tidak dikenali. Pilihan: rectangular/persegi, circular/bundar.",
            tank_shape
        );
    }
    ...
    match shape_lower.as_str() {
        "circular" | "bundar" => {
            ...
            out.push_str(&format!(
                "  Panjang weir (keliling) = {:.1} m\n",
                weir_length
            ));
            out.push_str(&format!(
                "  Weir loading = {:.1} m³/m/hari {}\n",
                weir_loading,
        ...
        "rectangular" | "persegi" => {
            ...
            out.push_str(&format!(
                ...
                if weir_loading <= weir_max {
                    ...
                    "❌ MELEBIHI — tambah weir"
                }
            ));
            if weir_loading > weir_max {
                out.push_str(&format!(
                    "  Panjang weir minimum = {:.1} m\n",
                    weir_length_min
                ));
            }
        }
